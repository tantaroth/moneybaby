...
.config(['$routeProvider', '$translateProvider', 'localStorageServiceProvider', 'LANG_DEFAULT', function($routeProvider, $translateProvider, localStorageServiceProvider, LANG_DEFAULT) {
    $routeProvider
        .when('/', {
 ...
 */
.run(['$rootScope', '$location', 'Auth', 'SECURED_ROUTES', 'loginRedirectPath', 'accountRedirectPath',
    function($rootScope, $location, Auth, SECURED_ROUTES, loginRedirectPath, accountRedirectPath) {
        ...
        function check(user) {
            if ((user && authRequired($location.path())) === null) {
                $location.path(loginRedirectPath);
            } else {
